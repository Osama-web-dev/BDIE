...
import { Activity, Shield, Users, Database, Settings, ActivitySquare, ShieldAlert, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function Sidebar() {
  ...
  const navItems = [
    ...
    { icon: <Users />, label: 'Digital Twins', href: '/users' },
    { icon: <Shield />, label: 'Risk Analysis', href: '/analysis' },
    { icon: <Database />, label: 'Data Lake', href: '/data' },
    { icon: <ActivitySquare />, label: 'Simulations', href: '/simulations' },
  ];
  ...
  return (
    ...
    >
      <button 
        ...
        className="absolute -right-3 top-8 w-6 h-6 bg-bdie-panel border border-bdie-border rounded-full flex items-center justify-center text-bdie-text-secondary hover:text-white z-30"
      >
        {isSidebarCollapsed ? <ChevronRight size={14} /> : <ChevronLeft size={14} />}
      </button>

      <div className={`flex items-center gap-3 w-full ${isSidebarCollapsed ? 'justify-center' : 'justify-start'}`}>
        <div className={`shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${isHighRisk ? 'bg-bdie-danger/20 text-bdie-danger box-glow-danger' : 'bg-bdie-accent/20 text-bdie-accent box-glow-accent'}`}>
          <ShieldAlert size={24} />
        </div>
        {!isSidebarCollapsed && (
